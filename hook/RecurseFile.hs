...
import System.Directory.Extra (listFilesInside, doesDirectoryExist, doesFileExist)
import Data.List.Extra (drop1, isPrefixOf)
import System.FilePath ((</>), isPathSeparator, takeFileName, takeExtension)
...
getFile include ignore (p:ath) exts t file = do
    ...
    if isDir then do
        ...
        if isFil then pure [p <\> file | not $ ignore $ p <\> file]
